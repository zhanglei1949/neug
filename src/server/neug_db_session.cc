...
neug::ReadTransaction NeugDBSession::GetReadTransaction() const {
  ...
  auto& slot = db_.snapshot_store().acquireSnapshot();
  return neug::ReadTransaction(slot, db_.snapshot_store(), *version_manager_,
                               ts);
}
...
neug::InsertTransaction NeugDBSession::GetInsertTransaction() {
  ...
  auto& slot = db_.snapshot_store().acquireSnapshot();
  return neug::InsertTransaction(slot, db_.snapshot_store(), alloc_, logger_,
                                 *version_manager_, ts);
}

neug::UpdateTransaction NeugDBSession::GetUpdateTransaction() {
  uint32_t ts = version_manager_->acquire_update_timestamp();
  auto cow_storage = db_.snapshot_store().currentSnapshot().Fork();
  return neug::UpdateTransaction(std::move(cow_storage), alloc_, logger_,
                                 *version_manager_, db_.snapshot_store(),
                                 pipeline_cache_, ts);
}

// const neug::PropertyGraph& NeugDBSession::graph() const { return db_.graph();
// }
...
neug::CompactTransaction NeugDBSession::GetCompactTransaction() {
  ...
  return neug::CompactTransaction(db_.snapshot_store(), logger_,
