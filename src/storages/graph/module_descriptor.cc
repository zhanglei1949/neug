...
std::optional<ModuleDescriptor> ModuleDescriptor::get_sub_module(
    const std::string& key) const {
  if (!sub_modules_impl_ || sub_modules_impl_->map.count(key) == 0) {
    return std::nullopt;
  }
  return sub_modules_impl_->map.at(key);
}

ModuleDescriptor ModuleDescriptor::get_sub_module_or_default(
    const std::string& key, const ModuleDescriptor& default_desc) const {
  if (!sub_modules_impl_ || sub_modules_impl_->map.count(key) == 0) {
    return default_desc;
  }
  return sub_modules_impl_->map.at(key);
