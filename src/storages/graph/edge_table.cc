...
void EdgeTable::Open(Checkpoint& ckp, const ModuleDescriptor& desc,
  ...
  if (!meta_->is_bundled()) {
    table_->Open(ckp, desc.get_sub_module_or_default("property_table"), level,
...
ModuleDescriptor EdgeTable::Dump(Checkpoint& ckp) {
  ...
  if (!meta_->is_bundled()) {
    ...
    desc.set("size", std::to_string(PropTableSize()));
  }
  return desc;
}
...
void EdgeTable::dropAndCreateNewUnbundledCSR(Checkpoint& ckp,
  ...
  auto next_oe_csr_path = ckp.runtime_dir() + "/" + ckp.create_runtime_object();
  auto next_ie_csr_path = ckp.runtime_dir() + "/" + ckp.create_runtime_object();
  auto next_table_prefix =
      ckp.runtime_dir() + "/" + ckp.create_runtime_object();
