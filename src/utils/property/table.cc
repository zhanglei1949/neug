...
std::unique_ptr<Table> Table::Fork(const Checkpoint& ckp,
  ...
  new_table->col_deleted_ = col_deleted_;
  new_table->name_ = name_;
  new_table->work_dir_ = ckp.runtime_dir();
  new_table->snapshot_dir_ = ckp.runtime_dir();
  new_table->touched_ = touched_;

  // new_table->columns_.resize(columns_.size(), nullptr);
  for (size_t i = 0; i < columns_.size(); ++i) {
    if (columns_[i] && (col_deleted_.empty() || !col_deleted_[i])) {
      // new_table->columns_[i] = columns_[i]->Fork(ckp, level);
      new_table->columns_.emplace_back(std::dynamic_pointer_cast<ColumnBase>(
          std::shared_ptr<Module>(columns_[i]->Fork(ckp, level))));
    }
  }
