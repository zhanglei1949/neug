...
std::vector<MemoryLevel> VStrats(size_t n, MemoryLevel s) {
  return std::vector<MemoryLevel>(n, s);
}
...
TEST(SchemaTest, AddVertexLabel_AddRenameDeleteVertexProperties_Physical) {
  ...
  auto v_strats = VStrats(v_types.size(), MemoryLevel::kInMemory);
  ...
  std::vector<MemoryLevel> add_strats = {MemoryLevel::kInMemory,
...
TEST(SchemaTest, AddEdgeLabel_AddRenameDeleteEdgeProperties_Physical) {
  ...
  // Prepare two vertex labels first
  {
    ...
    auto s = VStrats(t.size(), MemoryLevel::kInMemory);
...
TEST(SchemaTest, LogicalDeleteVertexProperties_HidesProperty) {
  ...
  auto pk = VPk(DataTypeId::kInt64, "id", 0);
  auto strats = VStrats(types.size(), MemoryLevel::kInMemory);
...
TEST(SchemaTest, LogicalDeleteEdgeProperties_HidesProperty) {
  ...
  auto vs = VStrats(vt.size(), MemoryLevel::kInMemory);
...
TEST(SchemaDumpTest, SchemaDumpWithMultipleEdgeTriplet) {
  ...
  auto person_strategies_ =
      VStrats(person_property_types_.size(), MemoryLevel::kInMemory);
  ...
  auto company_strategies_ =
      VStrats(company_property_types_.size(), MemoryLevel::kInMemory);
  ...
  auto edge_strategies_ =
      VStrats(edge_property_types_.size(), MemoryLevel::kInMemory);
...
class SchemaDeleteTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    person_strategies_ = {neug::MemoryLevel::kInMemory,
    ...
    company_strategies_ = {neug::MemoryLevel::kInMemory,
                           neug::MemoryLevel::kInMemory};
    ...
    edge_strategies_ = {neug::MemoryLevel::kInMemory};
  ...
  std::vector<neug::MemoryLevel> person_strategies_;
  ...
  std::vector<neug::MemoryLevel> company_strategies_;

  std::vector<neug::DataType> edge_property_types_;
  std::vector<std::string> edge_property_names_;
  std::vector<neug::MemoryLevel> edge_strategies_;
};
