...
Usage:
    python3 tools/python_bind/example/ldbc_snb_loader.py \
        --db-dir /path/to/db --data-root /path/to/social_network
"""
...
def current_checkpoint_id(db_dir: Path) -> Optional[int]:
    """Return the published checkpoint id when CURRENT is readable."""
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=2,
        help=(
            "COPY checkpoint strategy: 2 loads vertices then edges (default), "
            "1 commits all COPY statements together, and 31 keeps the "
...
def load_copy_statements(
    conn, copy: List[str], bulk_checkpoints: int, sample_interval_s: float
) -> Tuple[float, float]:
    """Execute COPY statements and return (statement time, bulk commit time)."""
    statement_total = 0.0
    commit_total = 0.0

    if bulk_checkpoints == 31:
        groups = [("auto-commit", copy)]
    elif bulk_checkpoints == 1:
        groups = [("all", copy)]
    else:
        groups = [("vertices", copy[:8]), ("edges", copy[8:])]

    statement_index = 0
    for group_name, statements in groups:
        group_memory = PeakRssTracker(sample_interval_s)
        group_memory.start()
        ...
        try:
            for stmt in statements:
                ...
                statement_total += elapsed
                print(
                    f"  [{statement_index}/{len(copy)}] {label} "
            ...
            if loader is not None:
                ...
                print(
                    f"  [{group_name}] bulk checkpoint committed in "
                    f"{commit_elapsed:.2f}s"
                )
        except Exception:
            if loader is not None and loader.active:
                try:
                    loader.rollback()
                except Exception:
                    # Keep the COPY failure as the primary loader error.
                    pass
            raise
        finally:
            group_memory.stop()
        ...
        print(
            f"  [{group_name}] session finished in {time.time() - group_start:.2f}s; "
            f"peak RSS {format_bytes(group_memory.peak_bytes())}"
        )

    return statement_total, commit_total
...
def main() -> None:
    ...
    db = neug.Database(
        str(db_dir), "w", buffer_strategy="M_LAZY", checkpoint_on_close=True
    )
    conn = db.connect()
    ...
    load_memory.start()
    checkpoint_before = current_checkpoint_id(db_dir)
    copy_phase_start = time.time()
    copy_total, bulk_commit_total = load_copy_statements(
        conn, copy, args.bulk_checkpoints, args.memory_sample_interval
    )
    copy_phase_elapsed = time.time() - copy_phase_start
    checkpoint_after = current_checkpoint_id(db_dir)
    load_memory.stop()
    print(
        f"[loader] COPY statement time: {copy_total:.2f}s " f"({len(copy)} statements)"
    )
    print(f"[loader] Bulk commit time: {bulk_commit_total:.2f}s")
    print(
        f"[loader] COPY phase total: {copy_phase_elapsed:.2f}s "
        f"(requested {args.bulk_checkpoints} checkpoint(s))"
    )
    if checkpoint_before is not None and checkpoint_after is not None:
        print(
            "[loader] Published checkpoint increment during COPY: "
            f"{checkpoint_after - checkpoint_before}"
        )
